...
public static unsafe class SelectStringHelper
{
    ...
    private static bool TryGetListEntryIconId(AtkComponentList* list, int entryIndex, out uint iconId)
    {
        ...
        // porting-note(api12/C): AtkComponentList.ListItem.IconId is a 7.5 field @0x10 (no named member on API12 FCS).
        // AtkComponentList is a stable core UI struct, so read raw @0x10. Runtime-pending; used to detect Triad NPC menu entries.
        iconId = *(uint*)((byte*)&list->ItemRendererList[entryIndex] + 0x10);
        return true;
    }
