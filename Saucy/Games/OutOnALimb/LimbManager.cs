...
public unsafe partial class LimbManager
{
    ...
    private static IGameObject? FindNearestLimbMachine() =>
        ObjectHelper.FindNearest(
            ArcadeMachineScopes.Limb,
            ObjectHelper.GetHorizontalEdgeDistance,
            static obj => obj.DataId == ArcadeMachineBaseIds.Limb[0] ? 2.5f : 4f);

    public static bool IsAnyLimbMachineInRange()
    {
        foreach (var obj in Svc.Objects)
        {
            if (Array.IndexOf(ArcadeMachineBaseIds.Limb, obj.DataId) < 0)
            {
                continue;
            }

            var maxDistance = obj.DataId == ArcadeMachineBaseIds.Limb[0] ? 2.5f : 4f;
