...
public unsafe class SliceIsRight : Module
{
    ...
    private static bool TryGetSliceHelperType(IGameObject gameObject, out uint helperType)
    {
        ...
        if (gameObject.ObjectKind == ObjectKind.EventObj)
        {
            ...
            // porting-note(api12/C): GameObject.GimmickId is a 7.5 field @0x80 (no named member in API12 FCS). Raw read, runtime-pending; comment above notes it's unused on current clients (BaseId path covers detection).
            var gimmickId = *(uint*)((byte*)gameObject.Address + 0x80);
