...
public unsafe partial class CuffACurAutomation
{
    ...
    private static IGameObject? FindNearestCuffMachine() =>
        ObjectHelper.FindNearest(
            ...
            static obj => obj.DataId == ArcadeMachineBaseIds.Cuff[0] ? 1f : 4f);

    public static bool IsAnyCuffMachineInRange()
    {
        foreach (var obj in Svc.Objects)
        {
            if (Array.IndexOf(ArcadeMachineBaseIds.Cuff, obj.DataId) < 0)
            {
                continue;
            }

            var maxDistance = obj.DataId == ArcadeMachineBaseIds.Cuff[0] ? 1f : 4f;
    ...
    public static void DrawDebug()
    {
        ...
        if (machine != null)
        {
            ImGuiEx.Text($"Nearest machine: {machine.Name.TextValue} ({machine.DataId}) dist={ObjectHelper.GetHorizontalEdgeDistance(machine):F2}");
