...
public static unsafe class AirForceAutomation
{
    ...
    public static void OnUpdate()
    {
        ...
        if (inDuty)
        {
            ...
            foreach (var x in Svc.Objects.OfType<IEventObj>().Where(x => x.DataId.EqualsAny<uint>(
                ...
                2015178,
                2015183
            )).Where(x => true /* TODO(api12): IEventObj.AnimationId is API15-only; can't filter to active-animation targets on 7.1 */).OrderBy(Player.DistanceTo))
            {
    ...
    public static void DrawDebug()
    {
        ...
        )).Where(x => true /* TODO(api12): IEventObj.AnimationId is API15-only; can't filter to active-animation targets on 7.1 */).OrderBy(Player.DistanceTo).Take(3).ToArray();
