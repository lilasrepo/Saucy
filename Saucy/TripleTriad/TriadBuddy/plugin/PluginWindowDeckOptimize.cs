...
public class PluginWindowDeckOptimize : Window, IDisposable
{
    ...
    private void DeckOptimizer_OnFoundDeck(TriadDeck deck, float estWinChance)
    {
        if (deck != null && deck.knownCards != null && deck.knownCards.Count == 5)
        {
            ...
            for (var idx = 0; idx < pendingCardIds.Length; idx++)
            {
    ...
    {
        var prevModDesc = regionModsDesc ?? "";
        ...
        if (npc != null)
        {
            ...
            foreach (var mod in gameRules)
            {
                if (mod != null)
                {
                    ...
                    if (Activator.CreateInstance(mod.GetType()) is TriadGameModifier modOb)
                    {
        ...
        var setupChanged = (npc != prevNpc) || (regionModsDesc != prevModDesc);
        if (setupChanged)
        {
    ...
    private void DrawCard(Vector2 drawPos, int cardId, int tooltipIdx, IDalamudTextureWrap cardBackgroundImage)
    {
        ...
        if (cardImage != null)
        {
            ...
            if (ImGui.IsItemHovered())
            {
                var isDeckEditorOpen = uiReaderDeckEdit?.IsVisible ?? false;
    ...
    private void UpdateTick()
    {
        ...
        if (pendingCardIds != null)
        {
            if (pendingCardsUpdateTimeRemaining <= 0.0f)
            {
                ...
                for (var idx = 0; idx < shownCardIds.Length; idx++)
                {
                    ...
                    if (cardOb != null)
                    {
                        ...
                        if (cardInfo != null)
                        {
                            var deckEditPageIdx = cardInfo.Collection[(int)GameCardCollectionFilter.DeckEditDefault].PageIndex;
                            shownCardTooltipsDeckEdit[idx] += "\n\n";
                            shownCardTooltipsDeckEdit[idx] += string.Format(locDeckEditPage ?? "", deckEditPageIdx + 1);

                            var collectionPageIdx = cardInfo.Collection[(int)GameCardCollectionFilter.All].PageIndex;
        ...
        if (isOptimizerRunning)
        {
            ...
            if (optimizerStatsTimeRemaining <= 0.0f)
            {
                ...
                var secondsRemaining = deckOptimizer.GetSecondsRemaining((int)(optimizerElapsedTime * 1000));
    ...
    private async void StartOptimizer()
    {
        if (!CanRunOptimizer())
        {
            Svc.Log.Error("Failed to start deck optimizer");
            return;
        }
        ...
        deckOptimizer.Initialize(npc, [.. regionMods], lockedCards);
        ...
        await deckOptimizer.Process(npc, [.. regionMods], lockedCards);
