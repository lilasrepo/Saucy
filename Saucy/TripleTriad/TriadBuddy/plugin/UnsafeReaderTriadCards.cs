    ...
    public UnsafeReaderTriadCards()
    {
        ...
        if (Svc.SigScanner != null)
        {
            try
            {
                ...
                // C-class walk-back: HEAD sig "E8 ?? ?? ?? ?? 84 C0 0F 94 C0 88 43 58 45 33 FF" targets game 7.5; TC client is 7.1
                IsNpcBeatenPtr = Svc.SigScanner.ScanText("40 53 48 83 ec 20 8d 82 fe ff dc ff");

                // UIState addr, use LEA opcode before calling IsTriadCardOwned, same function as described above
                // C-class walk-back: HEAD sig "E8 ?? ?? ?? ?? EB 35 8B FD" targets game 7.5; TC client is 7.1
                UIStatePtr = Svc.SigScanner.GetStaticAddressFromSig("48 8d 0d ?? ?? ?? ?? e8 ?? ?? ?? ?? 84 c0 74 0f 8b cb");
            }
            catch (Exception ex)
            {
