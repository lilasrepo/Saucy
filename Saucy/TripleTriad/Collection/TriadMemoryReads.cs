...
public static class TriadMemoryReads
{
    ...
    public static unsafe bool TryIsNpcBeatenOnce(int triadSheetRowId)
    {
        ...
        try
        {
            ...
            // porting-note(api12/B1): UIState.IsTripleTriadNpcBeaten is a 7.5 MemberFunction (sig absent on API12).
            // Stub to false → "hide beaten NPCs" filter treats all NPCs as not-yet-beaten. TODO(api12): wire the 7.1 sig.
            _ = triadSheetRowId;
            return false;
        }
        catch (Exception ex)
