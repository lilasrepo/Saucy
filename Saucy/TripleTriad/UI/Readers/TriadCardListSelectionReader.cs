...
internal static unsafe class TriadCardListSelectionReader
{
    ...
    public static bool IsMaskedUnownedSelection(AddonGSInfoCardList* addon, int displayCardId = -1)
    {
        ...
        if (displayCardId > 0)
        {
            var fromIcon = TriadCardDB.Get().TryGetCardIdFromIconId(GsCardListCompat.CardIconId(addon));
        ...
        if (TriadCardDB.Get().TryGetCardIdFromIconId(GsCardListCompat.CardIconId(addon)) < 0 &&
    ...
    private static int TryReadCardIdFromDetailPane(AddonGSInfoCardList* addon, int displayCardId)
    {
        ...
        var iconCardId = TriadCardDB.Get().TryGetCardIdFromIconId(GsCardListCompat.CardIconId(addon));
        if (iconCardId >= 0)
        {
            return iconCardId;
        }
    ...
    {
        if (GsCardListCompat.NumSideU(addon) == 0 &&
            GsCardListCompat.NumSideL(addon) == 0 &&
            GsCardListCompat.NumSideD(addon) == 0 &&
            GsCardListCompat.NumSideR(addon) == 0)
        {
            return -1;
        }

        var card = TriadCardDB.Get().Find(
            GsCardListCompat.NumSideU(addon),
            GsCardListCompat.NumSideL(addon),
            GsCardListCompat.NumSideD(addon),
            GsCardListCompat.NumSideR(addon),
            (ETriadCardType)GsCardListCompat.CardType(addon),
            (ETriadCardRarity)GsCardListCompat.CardRarity(addon));
        return card?.Id ?? -1;
    }
