...
// porting-note(api12/C): FCS 7.5's FFXIVClientStructs...Agent.CardListFilterMode enum and the
// AgentGoldSaucer.CardListFilterMode field (offset 0x10C) are absent from API12 (game-7.1) FCS. Mirror the
// enum locally; the field is read via raw offset in UIReaderTriadCardList.ReadAgentFilterMode (runtime-pending).
internal enum CardListFilterMode : byte
{
    All = 0,
    OwnedOnly = 1,
    MissingOnly = 2,
...
public unsafe class UIReaderTriadCardList : IUIReader
{
    ...
    public void OnAddonUpdate(nint addonPtr)
    {
        ...
        if (cachedAddonAgentPtr != nint.Zero)
        {
            ...
            newFilterMode = CardListFilterMapping.ToCollectionFilter(ReadAgentFilterMode(agent));
        }
        ...
        var selectionChanged = cachedState.pageIndex != newPageIndex ||
                               ...
                               cachedState.iconId != GsCardListCompat.CardIconId(addon) ||
                               cachedState.numU != GsCardListCompat.NumSideU(addon) ||
                               cachedState.numL != GsCardListCompat.NumSideL(addon) ||
                               cachedState.numD != GsCardListCompat.NumSideD(addon) ||
                               cachedState.numR != GsCardListCompat.NumSideR(addon) ||
                               cachedState.rarity != GsCardListCompat.CardRarity(addon) ||
                               cachedState.type != (byte)GsCardListCompat.CardType(addon) ||
    ...
    public bool SetPageAndGridView(int pageIndex, int cellIndex, int cardId = 0)
    {
        ...
        var filterMode = CardListFilterMapping.ToCollectionFilter(ReadAgentFilterMode(agent));
        var displayCardId = TriadCardListSelectionReader.TryParseCardIdFromDisplayLabel(addon);
    ...
    private void TickPendingCardNavigation(nint addonPtr)
    {
        ...
        {
            GsCardListCompat.SetRequestedPage(addon, pendingNavPage);
    ...
    private bool IsPendingNavigationComplete(AddonGSInfoCardList* addon)
    {
        ...
        if (TriadCardListSelectionReader.IconMatchesCard(GsCardListCompat.CardIconId(addon), pendingNavCardId))
        {
    ...
    {
        var hasSideStats = GsCardListCompat.NumSideU(addon) != 0 || GsCardListCompat.NumSideL(addon) != 0 || GsCardListCompat.NumSideD(addon) != 0 || GsCardListCompat.NumSideR(addon) != 0;
        if (!hasSideStats)
        {
            return true;
        }
        ...
        return GsCardListCompat.NumSideU(addon) == expectedCard.Sides[0] &&
               GsCardListCompat.NumSideL(addon) == expectedCard.Sides[1] &&
               GsCardListCompat.NumSideD(addon) == expectedCard.Sides[2] &&
               GsCardListCompat.NumSideR(addon) == expectedCard.Sides[3];
    }

    // porting-note(api12/C): API12 AgentGoldSaucer lacks the typed CardListFilterMode field (a 7.5 addition).
    // EditDeckSelectedPage (0x100) / EditDeckSelectedCardIndex (0x108) DO exist on API12, so the contiguous
    // 0x10C byte is read directly. Runtime-pending — verify the filter-mode read on the TC client.
    private static unsafe CardListFilterMode ReadAgentFilterMode(AgentGoldSaucer* agent)
        => *(CardListFilterMode*)((byte*)agent + 0x10C);
